...
RUN uv pip install .
# Install streamlit dependencies explicitly (including langchain-litellm if needed)
RUN uv pip install -r streamlit/requirements.txt
# Check if langchain-litellm needs to be installed manually if not in requirements
RUN uv pip install langchain-litellm || true
